    ...
    _createClass(Server, {
        submitTransaction: {
            ...
            value: function submitTransaction(transaction) {
                var tx = encodeURIComponent(isString(transaction) ? transaction : transaction.toEnvelope().toXDR().toString("base64"));
        ...
        _setAccountOptions: {
            value: function _setAccountOptions(endpoint, options, keypair) {
                ...
                var data = hash(JSON.stringify(signatureBase));
        ...
        setAgentLimits: {
            value: function setAgentLimits(accountId, asset_code, max_operation, daily_turnnover, monthly_turnover, keypair) {
